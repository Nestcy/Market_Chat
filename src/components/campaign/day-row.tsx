...
import { Camera, CheckCircle2, ImageOff, Loader2, Sparkles, ThumbsUp, Wand2 } from "lucide-react";
import {
  approveDay,
  generateDay,
  imageFallbackLabel,
...
export function DayRow({
  ...
  const inFlightMessage = useInFlightMessage(busy !== null);
            ...
            >
              {busy === "generate" ? (
                ...
                <>
                  ...
                  <span>{url ? "Regenerate image" : "Generate image"}</span>
                </>
              )}
            </button>
            <button
          ...
          {showFeedback ? (
            <div className="flex gap-1.5">
              <input
                ...
                className="min-w-0 flex-1 rounded-md border border-border bg-background/60 px-2 py-1 text-[11px] text-foreground outline-none focus:border-primary/50 disabled:opacity-50"
              />
              <button
                type="button"
                disabled={busy !== null || feedback.trim().length === 0}
                onClick={() => void act("tweak", () => tweakDay(campaignId, date, feedback.trim()))}
