...
export function ChatView({ onCampaignId, onCampaignIds, onPendingChange }: Props) {
  ...
  const inFlightMessage = useInFlightMessage(pending);
            ...
            <div className="space-y-5">
              ...
              {pending ? (
                <div className="flex items-start gap-2.5">
                  <div className="flex items-center gap-2 rounded-2xl rounded-bl-sm border border-border/80 bg-card/90 px-3.5 py-2.5 text-xs text-foreground/90 shadow-sm">
                    <Loader2 className="size-3.5 shrink-0 animate-spin text-primary" />
                    <span className="text-xs font-medium text-foreground/90 transition-opacity duration-300">
                      {inFlightMessage}
                    </span>
                  </div>
                </div>
