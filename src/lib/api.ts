...
async function request<T>(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return {
        ok: false,
        error: `Server responded ${response.status}.${detail || ` ${GENERIC_ERROR}`}`,
      };
    }
    ...
    if (err instanceof Error && err.name === "AbortError") {
      return {
        ok: false,
        error: "Request timed out — the agent may still be processing. Please try again.",
      };
    }
...
export function sendChat(messages: ChatTurn[]) {
  return request<ChatResponse>("/api/chat", json({ messages }), ACTION_TIMEOUT_MS);
}

export function fetchCampaignStatus(campaignId: string) {
  return request<CampaignStatus>(
    `/api/campaign/${id(campaignId)}/status`,
    undefined,
    READ_TIMEOUT_MS,
  );
}
...
export function approvePlan(campaignId: string) {
  return request<Record<string, unknown>>(
    `/api/campaign/${id(campaignId)}/plan/approve`,
    {
      method: "POST",
    },
    ACTION_TIMEOUT_MS,
...
export function fetchHub() {
  return request<{ events: HubEvent[] }>("/api/hub", undefined, READ_TIMEOUT_MS);
}

export function fetchCampaignTimeline(campaignId: string) {
  return request<{ events: HubEvent[] }>(
    `/api/campaign/${id(campaignId)}/timeline`,
    undefined,
    READ_TIMEOUT_MS,
  );
}
